...
#     return data
def generate_script(topic: str, host1: str, host2: str, exchanges: int, llm_model: str, ollama_url: str) -> list:
    prompt = f"""Write a podcast conversation between two hosts about: "{topic}"
...
"""
    ...
    response = requests.post(
        ...
        json={
            "model": llm_model,   # e.g. llama3.2:latest
            "stream": False,
            "messages": [
...
def main():
    ...
    parser.add_argument("--llm", default=None, help="LLM model for script generation")
    args = parser.parse_args()

    api_key = os.environ.get("SMALLEST_API_KEY")
    ollama_url = os.environ.get("OLLAMA_URL", "http://localhost:11434")
    llm_model = args.llm or os.environ.get("MODEL_NAME", "llama3.2:latest")
    ...
    print(f"Generating script with {llm_model} at {ollama_url}...\n")

    lines = generate_script(args.topic, args.host1_name, args.host2_name, args.exchanges, llm_model, ollama_url)
